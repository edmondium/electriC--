...
NODEPROTO *newnodeproto(CHAR *fname, LIBRARY *lib)
{
	...
	CHAR *ptr, *opt, *cpt, *nameend;
	//REGISTER CHAR save;
	CHAR save;
	//REGISTER INTBIG explicitversion;
	INTBIG explicitversion;
	...
	TECHNOLOGY *tech)
{
	...
	for(np = tech->firstnodeproto; np != NONODEPROTO; np = np->nextnodeproto)
	{
		if (np->primindex == pi)
		{
			...
			ttyputmsg(_((char*)"Index %ld of technology %s in use"), pi, tech->techname);
			return(NONODEPROTO);
		}
		if (namesame(fname, np->protoname) == 0)
		{
			//ttyputmsg(_("Error: Two technologies with same name: %s and %s"), fname, tech->techname);
			ttyputmsg(_((char*)"Error: Two technologies with same name: %s and %s"), fname, tech->techname);
			return(NONODEPROTO);
		}
	}
	if (pi <= 0)
	{
		ttyputmsg(_((char*)"Error: Component index must be positive but %s:%s has %ld"),
			tech->techname, fname, pi);
		return(NONODEPROTO);
	}
	if (sx < 0 || sy < 0)
	{
		ttyputmsg(_((char*)"Error: Component size must be positive, but %s:%s is %ldx%ld"),
			tech->techname, fname, sx, sy);
		return(NONODEPROTO);
	}
	for(pp = fname; *pp != 0; pp++) if (*pp <= ' ' || *pp == ':' || *pp >= 0177)
	{
		ttyputmsg(_((char*)"Error: Primitive name must not contain ':' and others but '%s:%s' does"),
			tech->techname, fname);
		return(NONODEPROTO);
	}
...
BOOLEAN killnodeproto(NODEPROTO *np)
{
	...
	for(pp = np->firstportproto; pp != NOPORTPROTO; pp = npt)
	{
		...
		if (killportproto(np, pp)) ttyputerr(_((char*)"killportproto error"));
	}
...
void db_insertnodeproto(NODEPROTO *np)
{
	...
	NODEPROTO *ent, *onp, *cellgrpchain1, *cellgrpchain2, *snp;
	...
	if (snp != NONODEPROTO)
	{
		...
		if (np->version > snp->version)
		{
			...
			if (onp == NONODEPROTO)
			{
				...
				ttyputmsg(M_((char*)"ERROR: Cannot replace newer version (v%ld) of cell %s"),
					np->version, np->protoname);					
				return;
			}
...
void db_retractnodeproto(NODEPROTO *np)
{
	...
	if (np->newestversion != np)
	{
		...
		if (lastnp != NONODEPROTO) lastnp->prevversion = np->prevversion;
			...
			else ttyputerr(_((char*)"Unusual version link in database"));
	} else
	{
...
void db_buildnodeprotohashtable(LIBRARY *lib)
{
	...
	INTBIG i, j, numnodeprotos, hashtablesize;
	//REGISTER NODEPROTO *np;
	NODEPROTO *np;
	...
	if (numnodeprotos != lib->numnodeprotos)
	{
		...
		ttyputmsg(_((char*)"Warning: number of nodeprotos in library %s corrected"), lib->libname);
...
NODEINST *descentparent(NODEPROTO *np, INTBIG *index, WINDOWPART *win, INTBIG *viewinfo)
{
	...
	if (db_descent_path_key == 0)
		...
		db_descent_path_key = makekey(x_((char*)"USER_descent_path"));
...
void sethierarchicalparent(NODEPROTO *np, NODEINST *parentni, WINDOWPART *win, INTBIG thisindex, INTBIG *viewinfo)
{
	...
	INTBIG i, j, k, len, *curlist, *newlist, index;
	INTBIG oneentry[25];
...
VIEW *newview(CHAR *vname, CHAR *svname)
{
	...
	if (namesamen(vname, x_((char*)"Schematic-Page-"), 15) == 0) v->viewstate |= MULTIPAGEVIEW;
...
void db_correctcellgroups(NODEPROTO *cell)
{
	...
	ttyputerr(_((char*)"Problems with cell group %s...repairing"), describenodeproto(cell));
	first = NONODEPROTO;
...
NODEPROTO *layoutview(NODEPROTO *np)
{
	...
	NODEPROTO *rnp, *laynp, *schnp, *uknnp;
	//REGISTER INTBIG i;
	INTBIG i;
	if (np == NONODEPROTO) return(NONODEPROTO);
...
PORTPROTO *equivalentport(NODEPROTO *np, PORTPROTO *pp, NODEPROTO *enp)
{
	...
	if (epp == NOPORTPROTO)
	{
		if (enp->cellview != el_iconview || (pp->userbits&BODYONLY) == 0)
			...
			ttyputerr(_((char*)"Warning: no port in cell %s corresponding to port %s in cell %s"),
				describenodeproto(enp), pp->protoname, describenodeproto(np));				
	}
...
INTBIG framepolys(NODEPROTO *np)
{
	...
	INTBIG xsections, ysections, tpolys, total;
	CHAR *size;
...
void framepoly(INTBIG boxnum, POLYGON *poly, NODEPROTO *np)
{
	...
	INTBIG pindex, xsections, ysections, dotitle, titleoffset;
	//REGISTER INTBIG xsecsize, ysecsize, schxsize, schysize, framewid, xlogobox, ylogobox;
	INTBIG xsecsize, ysecsize, schxsize, schysize, framewid, xlogobox, ylogobox;
	float wid, hei;
	//REGISTER VARIABLE *var;
	VARIABLE *var;
	static CHAR line[10];
	...
	} else
	{
		switch (boxnum-titleoffset-2)
		{
			...
			case 3:			/* designer name */
				...
				var = getval((INTBIG)np->lib, VLIBRARY, VSTRING, x_((char*)"USER_drawing_designer_name"));
				if (var == NOVARIABLE)
					//var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_("USER_drawing_designer_name"));
					var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_((char*)"USER_drawing_designer_name"));
			...
			case 9:			/* project name */
				...
				var = getval((INTBIG)np->lib, VLIBRARY, VSTRING, x_((char*)"USER_drawing_project_name"));
				if (var == NOVARIABLE)
					//var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_("USER_drawing_project_name"));
					var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_((char*)"USER_drawing_project_name"));
				if (var == NOVARIABLE) break;
				poly->xv[0] = poly->xv[3] = schxsize/2 - framewid - xlogobox;
				poly->xv[1] = poly->xv[2] = schxsize/2 - framewid;
			...
			case 11:		/* company name */
				...
				var = getval((INTBIG)np->lib, VLIBRARY, VSTRING, x_((char*)"USER_drawing_company_name"));
				if (var == NOVARIABLE)
					//var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_("USER_drawing_company_name"));
					var = getval((INTBIG)us_tool, VTOOL, VSTRING, x_((char*)"USER_drawing_company_name"));
				if (var == NOVARIABLE) break;
				poly->xv[0] = poly->xv[3] = schxsize/2 - framewid - xlogobox;
				poly->xv[1] = poly->xv[2] = schxsize/2 - framewid;
...
NODEPROTO *copynodeproto(NODEPROTO *fromnp, LIBRARY *tolib, CHAR *toname, BOOLEAN useexisting)
{
	...
	BOOLEAN maysubstitute;
	//REGISTER INTBIG i, failures, res;
	INTBIG i, failures, res;
	//REGISTER PORTPROTO *pp, *ppt, *p1, *p2;
	PORTPROTO *pp, *ppt, *p1, *p2;
	...
	if (destlib != NOLIBRARY)
	{
		...
		for(ni = fromnp->firstnodeinst; ni != NONODEINST; ni = ni->nextnodeinst)
		{
			...
			for(pi = ni->firstportarcinst; pi != NOPORTARCINST; pi = pi->nextportarcinst)
			{
				...
				if (ppt == NOPORTPROTO)
				{
					ttyputerr(_((char*)"Cannot use subcell %s in library %s: exports don't match"),
	...
	for(ai = fromnp->firstarcinst; ai != NOARCINST; ai = ai->nextarcinst)
	{
		...
		for(i=0; i<2; i++)
		{
			...
			if (opt[i] == NOPORTPROTO)
				...
				ttyputerr(_((char*)"Error: no port for %s arc on %s node"), describearcproto(ai->proto),
					describenodeproto(ai->end[i].nodeinst->proto));					
		}
	...
	for(pp = fromnp->firstportproto; pp != NOPORTPROTO; pp = pp->nextportproto)
	{
		...
		if (pp->subportproto != p2)
			...
			ttyputerr(_((char*)"Error: no port on %s cell"), describenodeproto(pp->subnodeinst->proto));
	...
	if (failures != 0)
		...
		ttyputmsg(_((char*)"WARNING: cross-library copy of cell %s deleted %ld variables"),
			describenodeproto(fromnp), failures);
	...
	INTBIG totype, NODEPROTO *cell, LIBRARY *destlib)
{
	...
	INTBIG key, addr, type, len, *newaddr;
	INTSML *numvar;
	VARIABLE **firstvar;
	//REGISTER VARIABLE *var;
	VARIABLE *var;
	...
	BOOLEAN allowmissingports)
{
	...
	PORTPROTO *opt, **newportprotos, **newexpportprotos;
	...
	for(pi = ni->firstportarcinst; pi != NOPORTARCINST; pi = pi->nextportarcinst)
	{
		...
		if (opt == NOPORTPROTO)
		{
			...
			if (db_printerrors)
				...
				ttyputmsg(_((char*)"No port on new node corresponds to old port: %s"), pi->proto->protoname);
			db_killnodeinst(newno);
		...
		if (opt->connects[i] == NOARCPROTO)
		{
			...
			if (db_printerrors)
				...
				ttyputmsg(_((char*)"%s arc on old port %s cannot connect to new port %s"),
					describearcinst(ai), pi->proto->protoname, opt->protoname);					
			db_killnodeinst(newno);
			return((NODEINST *)db_error(DBPORTMM|DBREPLACENODEINST));
		}
		...
		if (opt == NOPORTPROTO)
		{
			if (db_printerrors)
				/*ttyputmsg(_("No port on new node corresponds to old port: %s"),
					pe->proto->protoname);*/
				ttyputmsg(_((char*)"No port on new node corresponds to old port: %s"),
					pe->proto->protoname);					
			db_killnodeinst(newno);
			return((NODEINST *)db_error(DBPORTMM|DBREPLACENODEINST));
		}
				...
				if (ai->end[i].portarcinst == pi)
				{
					...
					if (endportproto[i] == NOPORTPROTO)
					{
						if (!allowmissingports)
							...
							ttyputerr(_((char*)"Cannot re-connect %s arc"), describearcinst(ai));
						break;
					}
		...
		for(pi = ni->firstportarcinst; pi != NOPORTARCINST; pi = pi->nextportarcinst)
		{
			...
			if (conn[i] == NOARCPROTO)
			{
				if (db_printerrors)
					...
					ttyputmsg(_((char*)"%s arc in cell %s cannot connect to port %s"),
						describearcinst(pi->conarcinst), describenodeproto(ni->parent), pp->protoname);						
				return(TRUE);
			}
		}
	...
	for(ni = pp->parent->firstinst; ni != NONODEINST; ni = ni->nextinst)
	{
		...
		for(pe = ni->firstportexpinst; pe != NOPORTEXPINST; pe = pe->nextportexpinst)
		{
			...
			if (pe->exportproto->userbits != pp->userbits)
			{
				...
				setval((INTBIG)pe->exportproto, VPORTPROTO, x_((char*)"userbits"),
					pp->userbits, VINTEGER);					
			}
