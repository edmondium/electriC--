...
{
	INTBIG hipart, lonum, loden, retval, units;
	INTBIG neg;
	CHAR *ptr;
	double scale;
...
CHAR *displayedunits(float value, INTBIG unittype, INTBIG unitscale)
{
	...
	switch (pureunit)
	{
		...
		case PUREUNITMEGA:		/* mega:  x 1000000 */
			value /= 1000000.0f;
			postfix = x_((char*)"meg");		/* SPICE wants "x" */
			break;
...
float figureunits(CHAR *str, INTBIG unittype, INTBIG unitscale)
{
	...
	if (len > 1)
	{
		if (namesame(&str[len-1], x_((char*)"g")) == 0) pureunit = PUREUNITGIGA;
		if (namesame(&str[len-1], x_((char*)"k")) == 0) pureunit = PUREUNITKILO;
		if (namesame(&str[len-1], x_((char*)"m")) == 0) pureunit = PUREUNITMILLI;
		if (namesame(&str[len-1], x_((char*)"u")) == 0) pureunit = PUREUNITMICRO;
		if (namesame(&str[len-1], x_((char*)"n")) == 0) pureunit = PUREUNITNANO;
		if (namesame(&str[len-1], x_((char*)"p")) == 0) pureunit = PUREUNITPICO;
		if (namesame(&str[len-1], x_((char*)"f")) == 0) pureunit = PUREUNITFEMTO;
	}
	if (len > 3)
	{
		if (namesame(&str[len-3], x_((char*)"meg")) == 0) pureunit = PUREUNITMEGA;
	}
	switch (pureunit)
	{
		case PUREUNITGIGA:		/* giga:  x 1000000000 */
...
NODEPROTO *getnodeproto(CHAR *initline)
{
	NODEPROTO *np;
	TECHNOLOGY *tech, *t;
	INTBIG wantversion, save, saidtech, saidlib;
	LIBRARY *lib, *l;
	VIEW *wantview, *v;
	CHAR *pt, *line, *nameend, *viewname;
	void *infstr;
...
static COMCOMP db_technologyp = {NOKEYWORD, topoftechs, nexttechs, NOPARAMS,
	0, x_((char*)" \t"), M_((char*)"technology"), 0};
...
{
	INTBIG len, i, l, k, c1, c2, addr;
	INTBIG count;
	NETWORK *net;
	CHAR *pt, save, *lastslash;
...
{
	{N_((char*)"none"),           x_((char*)"ALLOFF"),  ALLOFF},
	{N_((char*)"transparent-1"),  x_((char*)"COLORT1"), COLORT1},
	{N_((char*)"transparent-2"),  x_((char*)"COLORT2"), COLORT2},
	{N_((char*)"transparent-3"),  x_((char*)"COLORT3"), COLORT3},
	{N_((char*)"transparent-4"),  x_((char*)"COLORT4"), COLORT4},
	{N_((char*)"transparent-5"),  x_((char*)"COLORT5"), COLORT5},
	{N_((char*)"overlappable-1"), x_((char*)"COLORT1"), COLORT1},
	{N_((char*)"overlappable-2"), x_((char*)"COLORT2"), COLORT2},
	{N_((char*)"overlappable-3"), x_((char*)"COLORT3"), COLORT3},
	{N_((char*)"overlappable-4"), x_((char*)"COLORT4"), COLORT4},
	{N_((char*)"overlappable-5"), x_((char*)"COLORT5"), COLORT5},
	{N_((char*)"white"),          x_((char*)"WHITE"),   WHITE},
	{N_((char*)"black"),          x_((char*)"BLACK"),   BLACK},
	{N_((char*)"red"),            x_((char*)"RED"),     RED},
	{N_((char*)"blue"),           x_((char*)"BLUE"),    BLUE},
	{N_((char*)"green"),          x_((char*)"GREEN"),   GREEN},
	{N_((char*)"cyan"),           x_((char*)"CYAN"),    CYAN},
	{N_((char*)"magenta"),        x_((char*)"MAGENTA"), MAGENTA},
	{N_((char*)"yellow"),         x_((char*)"YELLOW"),  YELLOW},
	{N_((char*)"gray"),           x_((char*)"GRAY"),    GRAY},
	{N_((char*)"orange"),         x_((char*)"ORANGE"),  ORANGE},
	{N_((char*)"purple"),         x_((char*)"PURPLE"),  PURPLE},
	{N_((char*)"brown"),          x_((char*)"BROWN"),   BROWN},
	{N_((char*)"light-gray"),     x_((char*)"LGRAY"),   LGRAY},
	{N_((char*)"dark-gray"),      x_((char*)"DGRAY"),   DGRAY},
	{N_((char*)"light-red"),      x_((char*)"LRED"),    LRED},
	{N_((char*)"dark-red"),       x_((char*)"DRED"),    DRED},
	{N_((char*)"light-green"),    x_((char*)"LGREEN"),  LGREEN},
	{N_((char*)"dark-green"),     x_((char*)"DGREEN"),  DGREEN},
	{N_((char*)"light-blue"),     x_((char*)"LBLUE"),   LBLUE},
	{N_((char*)"dark-blue"),      x_((char*)"DBLUE"),   DBLUE},
	{NULL, NULL, 0}
};
...
INTBIG parse(CHAR *keyword, COMCOMP *list, BOOLEAN noise)
{
	...
	if (i == 0)
	{
		if (noise != 0) ttyputerr(_((char*)"Unknown command: %s"), keyword);
		return(-1);
	}
	...
	if (noise != 0)
	{
		...
		ttyputerr(_((char*)"%s ambiguous:%s"), keyword, returninfstr(infstr));
	}
	return(-1);
}
...
CHAR *ntdescribenodeinst(NODEINST *ni)
{
	...
	if (ni == NONODEINST) return(x_((char*)"***NONODEINST***"));
...
CHAR *describenodeinst(NODEINST *ni)
{
	...
	if (len > db_descnioutputsize[db_descninodeswitch])
	{
		...
		if (db_descnioutput[db_descninodeswitch] == 0) return(x_((char*)""));
...
CHAR *describearcinst(ARCINST *ai)
{
	...
	if (ai == NOARCINST) return(x_((char*)"***NOARCINST***"));
	...
	if (len > db_descaioutputsize[db_descaiarcswitch])
	{
		...
		if (db_descaioutput[db_descaiarcswitch] == 0) return(x_((char*)""));
...
CHAR *nldescribenodeproto(NODEPROTO *np)
{
	...
	if (np == NONODEPROTO) return(x_((char*)"***NONODEPROTO***"));
...
CHAR *describenodeproto(NODEPROTO *np)
{
	...
	} else
	{
		...
		if (np->protoname == 0) return(x_((char*)"***BOGUS***"));
	...
	if (len > db_descnpoutputsize[db_descnpnodeswitch])
	{
		...
		if (db_descnpoutput[db_descnpnodeswitch] == 0) return(x_((char*)""));
...
CHAR *describearcproto(ARCPROTO *ap)
{
	...
	if (ap == NOARCPROTO) return(x_((char*)"***NOARCPROTO***"));
	...
	if (len > db_descapoutputsize[db_descaparcswitch])
	{
		...
		if (db_descapoutput[db_descaparcswitch] == 0) return(x_((char*)""));
...
{
	if (geom == NOGEOM) return(x_((char*)"***NOGEOM***"));
...
CHAR *describenetwork(NETWORK *net)
{
	...
	if (net == NONETWORK) return(x_((char*)"***NONETWORK***"));

	if (net->globalnet >= 0)
	{
		infstr = initinfstr();
		addstringtoinfstr(infstr, _((char*)"Global"));
		addtoinfstr(infstr, '-');
		if (net->globalnet == GLOBALNETGROUND) addstringtoinfstr(infstr, _((char*)"Ground")); else
		if (net->globalnet == GLOBALNETPOWER) addstringtoinfstr(infstr, _((char*)"Power")); else
		{
			np = net->parent;
			if (net->globalnet >= np->globalnetcount)
				addstringtoinfstr(infstr, _((char*)"UNKNOWN")); else
...
CHAR *describeportbits(INTBIG userbits)
{
	...
	{
		case INPORT:      return(_((char*)"Input"));
		case OUTPORT:     return(_((char*)"Output"));
		case BIDIRPORT:   return(_((char*)"Bidirectional"));
		case PWRPORT:     return(_((char*)"Power"));
		case GNDPORT:     return(_((char*)"Ground"));
		case CLKPORT:     return(_((char*)"Clock"));
		case C1PORT:      return(_((char*)"Clock Phase 1"));
		case C2PORT:      return(_((char*)"Clock Phase 2"));
		case C3PORT:      return(_((char*)"Clock Phase 3"));
		case C4PORT:      return(_((char*)"Clock Phase 4"));
		case C5PORT:      return(_((char*)"Clock Phase 5"));
		case C6PORT:      return(_((char*)"Clock Phase 6"));
		case REFOUTPORT:  return(_((char*)"Reference Output"));
		case REFINPORT:   return(_((char*)"Reference Input"));
		case REFBASEPORT: return(_((char*)"Reference Base"));
	}
	return(x_((char*)"Unknown"));
}
...
CHAR *describeobject(INTBIG addr, INTBIG type)
{
	...
	switch (type&VTYPE)
	{
		case VNODEINST:
			ni = (NODEINST *)addr;
			formatinfstr(infstr, x_((char*)"NodeInstance(%s)"), describenodeinst(ni));
			break;
		case VNODEPROTO:
			np = (NODEPROTO *)addr;
			if (np->primindex == 0) formatinfstr(infstr, x_((char*)"Cell(%s)"), describenodeproto(np)); else
				formatinfstr(infstr, x_((char*)"Primitive(%s)"), describenodeproto(np));
			break;
		case VPORTARCINST:
			pi = (PORTARCINST *)addr;
			if (pi == NOPORTARCINST) addstringtoinfstr(infstr, x_((char*)"PortArcInstance(NULL)")); else
				formatinfstr(infstr, x_((char*)"PortArcInstance(%ld)"), (INTBIG)pi);
			break;
		case VPORTEXPINST:
			pe = (PORTEXPINST *)addr;
			if (pe == NOPORTEXPINST) addstringtoinfstr(infstr, x_((char*)"PortExpInstance(NULL)")); else
				formatinfstr(infstr, x_((char*)"PortExpInstance(%ld)"), (INTBIG)pe);
			break;
		case VPORTPROTO:
			pp = (PORTPROTO *)addr;
			if (pp == NOPORTPROTO) addstringtoinfstr(infstr, x_((char*)"PortPrototype(NULL)")); else
				formatinfstr(infstr, x_((char*)"PortPrototype(%s)"), pp->protoname);
			break;
		case VARCINST:
			ai = (ARCINST *)addr;
			formatinfstr(infstr, x_((char*)"ArcInstance(%s)"), describearcinst(ai));
			break;
		case VARCPROTO:
			ap = (ARCPROTO *)addr;
			formatinfstr(infstr, x_((char*)"ArcPrototype(%s:%s)"), ap->tech->techname, ap->protoname);
			break;
		case VGEOM:
			geom = (GEOM *)addr;
			if (geom == NOGEOM) addstringtoinfstr(infstr, x_((char*)"Geom(NULL)")); else
				formatinfstr(infstr, x_((char*)"Geom(%ld)"), (INTBIG)geom);
			break;
		case VLIBRARY:
			lib = (LIBRARY *)addr;
			if (lib == NOLIBRARY) addstringtoinfstr(infstr, x_((char*)"Library(NULL)")); else
				formatinfstr(infstr, x_((char*)"Library(%s)"), lib->libname);
			break;
		case VTECHNOLOGY:
			tech = (TECHNOLOGY *)addr;
			if (tech == NOTECHNOLOGY) addstringtoinfstr(infstr, x_((char*)"Technology(NULL)")); else
				formatinfstr(infstr, x_((char*)"Technology(%s)"), tech->techname);
			break;
		case VTOOL:
			tool = (TOOL *)addr;
			if (tool == NOTOOL) addstringtoinfstr(infstr, x_((char*)"Tool(NULL)")); else
				formatinfstr(infstr, x_((char*)"Tool(%s)"), tool->toolname);
			break;
		case VRTNODE:
			rtn = (RTNODE *)addr;
			if (rtn == NORTNODE) addstringtoinfstr(infstr, x_((char*)"Rtnode(NULL)")); else
				formatinfstr(infstr, x_((char*)"Rtnode(%ld)"), (INTBIG)rtn);
			break;
		case VVIEW:
			v = (VIEW *)addr;
			if (v == NOVIEW) addstringtoinfstr(infstr, x_((char*)"View(NULL)")); else
				formatinfstr(infstr, x_((char*)"View(%s)"), v->viewname);
			break;
		case VWINDOWPART:
			win = (WINDOWPART *)addr;
			if (win != NOWINDOWPART) formatinfstr(infstr, x_((char*)"WindowPart(%s)"), win->location); else
				formatinfstr(infstr, x_((char*)"WindowPart(%ld)"), (INTBIG)win);
			break;
		case VGRAPHICS:
			gra = (GRAPHICS *)addr;
			if (gra == NOGRAPHICS) addstringtoinfstr(infstr, x_((char*)"Graphics(NULL)")); else
				formatinfstr(infstr, x_((char*)"Graphics(%ld)"), (INTBIG)gra);
			break;
		case VCONSTRAINT:
			con = (CONSTRAINT *)addr;
			if (con == NOCONSTRAINT) addstringtoinfstr(infstr, x_((char*)"Constraint(NULL)")); else
				formatinfstr(infstr, x_((char*)"Constraint(%s)"), con->conname);
			break;
		case VWINDOWFRAME:
			wf = (WINDOWFRAME *)addr;
			formatinfstr(infstr, x_((char*)"WindowFrame(%ld)"), (INTBIG)wf);
			break;
		case VPOLYGON:
			poly = (POLYGON *)addr;
			formatinfstr(infstr, x_((char*)"Polygon(%ld)"), (INTBIG)poly);
			break;
		default:
			addstringtoinfstr(infstr, x_((char*)"UNKNOWN(?)"));
			break;
	}
	return(returninfstr(infstr));
}
...
{
	INTBIG pixperlam, lambda, height;
	LIBRARY *lib;
...
{
	VARIABLE *txtvar;
	INTBIG dx, dy, goleft, goright, goup, godown;
	INTBIG *defdescript;
	NODEINST *ni;
	ARCINST *ai;
	...
	if (user_default_text_style_key == 0)
		user_default_text_style_key = makekey(x_((char*)"USER_default_text_style"));
	if (user_default_text_smart_style_key == 0)
		user_default_text_smart_style_key = makekey(x_((char*)"USER_default_text_smart_style"));
...
void defaulttextsize(INTBIG texttype, UINTBIG *descript)
{
	...
	if (node_size_key == 0)
		node_size_key = makekey(x_((char*)"USER_default_node_text_size"));
	if (arc_size_key == 0)
		arc_size_key = makekey(x_((char*)"USER_default_arc_text_size"));
	if (export_size_key == 0)
		export_size_key = makekey(x_((char*)"USER_default_export_text_size"));
	if (nonlayout_size_key == 0)
		nonlayout_size_key = makekey(x_((char*)"USER_default_nonlayout_text_size"));
	if (instance_size_key == 0)
		instance_size_key = makekey(x_((char*)"USER_default_instance_text_size"));
	if (cell_size_key == 0)
		cell_size_key = makekey(x_((char*)"USER_default_facet_text_size"));
...
{
	INTBIG scale, realx, realy, negx, negy;
...
CHAR *describedisplayedvariable(VARIABLE *var, INTBIG aindex, INTBIG purpose)
{
	...
	if (var == NOVARIABLE)
	{
		if (purpose <= 0) return(x_((char*)"***UNKNOWN***")); else return(x_((char*)""));
	}
	...
	if ((var->type&VDISPLAY) != 0)
	{
		...
		switch (TDGETDISPPART(var->textdescript))
		{
			case VTDISPLAYNAMEVALUE:
				if (len > 1 && aindex >= 0)
				{
					formatinfstr(infstr, x_((char*)"%s[%ld]="), name, aindex);
				} else
				{
					formatinfstr(infstr, x_((char*)"%s="), name);
				}
				break;
			case VTDISPLAYNAMEVALINH:
				...
				if (parval == NOVARIABLE) parstr = x_((char*)"?"); else
					parstr = describevariable(parval, -1, purpose);
				if (len > 1 && aindex >= 0)
				{
					formatinfstr(infstr, x_((char*)"%s[%ld]=%s;def="), name, aindex, parstr);
				} else
				{
					formatinfstr(infstr, x_((char*)"%s=%s;def="), name, parstr);
				}
				break;
...
void db_makestringvar(INTBIG type, INTBIG addr, INTBIG purpose, INTBIG units, void *infstr)
{
	...
	switch (type&VTYPE)
	{
		...
		case VBOOLEAN:
			...
			return;
		case VADDRESS:
			if (purpose == 0) addstringtoinfstr(infstr, x_((char*)"Address="));
		...
		case VSTRING:
			...
			{
				addstringtoinfstr(infstr, x_((char*)"***NOSTRING***"));
				return;
			}
		...
		case VNODEINST:
			...
			if (ni == NONODEINST)
			{
				addstringtoinfstr(infstr, x_((char*)"***NONODEINST***"));
				return;
			}
			if (purpose == 0)
			{
		...
		case VPORTARCINST:
			...
			if (pi == NOPORTARCINST)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOPORTARCINST***"));
				return;
			}
		...
		case VPORTEXPINST:
			...
			if (pe == NOPORTEXPINST)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOPORTEXPINST***"));
				return;
			}
		...
		case VPORTPROTO:
			...
			if (pp == NOPORTPROTO)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOPORTPROTO***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VARCINST:
			...
			if (ai == NOARCINST)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOARCINST***"));
				return;
			}
		...
		case VGEOM:
			...
			if (geom == NOGEOM)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOGEOM***"));
				return;
			}
		...
		case VLIBRARY:
			...
			if (lib == NOLIBRARY)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOLIBRARY***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VTECHNOLOGY:
			...
			{
				addstringtoinfstr(infstr, x_((char*)"***NOTECHNOLOGY***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VTOOL:
			...
			if (tool == NOTOOL)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOTOOL***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VRTNODE:
			...
			if (rtn == NORTNODE)
			{
				addstringtoinfstr(infstr, x_((char*)"***NORTNODE***"));
				return;
			}
		...
		case VNETWORK:
			...
			if (net == NONETWORK)
			{
				addstringtoinfstr(infstr, x_((char*)"***NONETWORK***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VVIEW:
			...
			if (view == NOVIEW)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOVIEW***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VWINDOWPART:
			...
			if (win == NOWINDOWPART)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOWINDOWPART***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VGRAPHICS:
			...
			if (gra == NOGRAPHICS)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOGRAPHICS***"));
				return;
			}
		...
		case VCONSTRAINT:
			...
			if (con == NOCONSTRAINT)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOCONSTRAINT***"));
				return;
			}
			if (purpose <= 0)
			{
		...
		case VWINDOWFRAME:
			...
			if (wf == NOWINDOWFRAME)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOWINDOWFRAME***"));
				return;
			}
		...
		case VPOLYGON:
			...
			if (poly == NOPOLYGON)
			{
				addstringtoinfstr(infstr, x_((char*)"***NOPOLYGON***"));
				return;
			}
...
CHAR *unitsname(INTBIG units)
{
	...
	{
		case INTUNITHNM:   return(_((char*)"half-nanometer"));
		case INTUNITHDMIC: return(_((char*)"half-decimicron"));
	}
	return(x_((char*)"?"));
}
...
{
	N_((char*)"Contour gather"),		/* STOPREASONCONTOUR */
	N_((char*)"DRC"),					/* STOPREASONDRC */
	N_((char*)"Playback"),				/* STOPREASONPLAYBACK */
	N_((char*)"Binary"),				/* STOPREASONBINARY */
	N_((char*)"CIF"),					/* STOPREASONCIF */
	N_((char*)"DXF"),					/* STOPREASONDXF */
	N_((char*)"EDIF"),					/* STOPREASONEDIF */
	N_((char*)"VHDL"),					/* STOPREASONVHDL */
	N_((char*)"Compaction"),			/* STOPREASONCOMPACT */
	N_((char*)"ERC"),					/* STOPREASONERC */
	N_((char*)"Check-in"),				/* STOPREASONCHECKIN */
	N_((char*)"Lock wait"),			/* STOPREASONLOCK */
	N_((char*)"Network comparison"),	/* STOPREASONNCC */
	N_((char*)"Port exploration"),		/* STOPREASONPORT */
	N_((char*)"Routing"),				/* STOPREASONROUTING */
	N_((char*)"Silicon compiler"),		/* STOPREASONSILCOMP */
	N_((char*)"Display"),				/* STOPREASONDISPLAY */
	N_((char*)"Simulation"),			/* STOPREASONSIMULATE */
	N_((char*)"Deck generation"),		/* STOPREASONDECK */
	N_((char*)"SPICE"),				/* STOPREASONSPICE */
	N_((char*)"Check"),				/* STOPREASONCHECK */
	N_((char*)"Array"),				/* STOPREASONARRAY */
	N_((char*)"Iteration"),			/* STOPREASONITERATE */
	N_((char*)"Replace"),				/* STOPREASONREPLACE */
	N_((char*)"Spread"),				/* STOPREASONSPREAD */
	N_((char*)"Execution"),			/* STOPREASONEXECUTE */
	N_((char*)"Command file"),			/* STOPREASONCOMFILE */
	N_((char*)"Selection"),			/* STOPREASONSELECT */
	N_((char*)"Tracking"),				/* STOPREASONTRACK */
	N_((char*)"Network evaluation"),	/* STOPREASONNETWORK */
	0
};
...
{
	CHAR *pt1, *pt2, pt1save, pt2save, *number1, *number2,
		*nameend1, *nameend2;
	INTBIG pt1index, pt2index, pt1number, pt2number;
	INTBIG compare;
...
{
	INTBIG mon;
	static CHAR *name[] = {x_((char*)"Jan"), x_((char*)"Feb"), x_((char*)"Mar"), x_((char*)"Apr"), x_((char*)"May"), x_((char*)"Jun"), x_((char*)"Jul"),
		x_((char*)"Aug"), x_((char*)"Sep"), x_((char*)"Oct"), x_((char*)"Nov"), x_((char*)"Dec")};
...
{
	return(efopen(truepath(name), x_((char*)"a")));
}
