	...
	INTBIG *deltahxs, INTBIG *deltahys, INTBIG *deltarots, INTBIG *deltatranss)
{
	...
	INTBIG i, deltalx, deltaly, deltahx, deltahy;
	//REGISTER INTBIG deltarot, deltatrans;
	INTBIG deltarot, deltatrans;
	//REGISTER NODEINST *ni;
	NODEINST *ni;
	if (!db_donextchangequietly && !db_dochangesquietly)
	{
	...
	NODEPROTO *parnt)
{
	...
	if (pA->connects[i] == NOARCPROTO)
	{
		...
		if (db_printerrors)
			...
			ttyputmsg(_((char*)"Port %s of node %s in cell %s cannot connect to arc %s"), pA->protoname,
				describenodeinst(nA), describenodeproto(parnt), describearcproto(typ));				
		db_donextchangequietly = FALSE;
		return(NOARCINST);
	}
	...
	if (pB->connects[i] == NOARCPROTO)
	{
		...
		if (db_printerrors)
			...
			ttyputmsg(_((char*)"Port %s of node %s in cell %s cannot connect to arc %s"), pB->protoname,
				describenodeinst(nB), describenodeproto(parnt), describearcproto(typ));				
		db_donextchangequietly = FALSE;
		return(NOARCINST);
	}
	...
	if (!isinside(xA, yA, poly))
	{
		...
		if (db_printerrors)
		{
			...
			ttyputmsg(_((char*)"Point (%s,%s) not inside port %s of node %s in cell %s"), latoa(xA, lambda),
				latoa(yA, lambda), pA->protoname, describenodeinst(nA), describenodeproto(parnt));				
		}
		db_donextchangequietly = FALSE;
		freepolygon(poly);
	...
	if (!isinside(xB, yB, poly))
	{
		...
		if (db_printerrors)
		{
			...
			ttyputmsg(_((char*)"Point (%s,%s) not inside port %s of node %s in cell %s"), latoa(xB, lambda),
				latoa(yB, lambda), pB->protoname, describenodeinst(nB), describenodeproto(parnt));				
		}
		db_donextchangequietly = FALSE;
		freepolygon(poly);
	...
	INTBIG deltax2, INTBIG deltay2)
{
	...
	INTBIG oldwid, oldlen, oldx0, oldy0, oldx1, oldy1;
...
PORTPROTO *db_newprimportproto(NODEPROTO *np, ARCPROTO **arcs, CHAR *name)
{
	...
	for(ptr = name; *ptr != 0; ptr++) if (*ptr <= ' ' || *ptr >= 0177)
	{
		...
		ttyputmsg(_((char*)"Port '%s' on primitive '%s' has bad name (spaces?)"), name, np->protoname);
		return(NOPORTPROTO);
	}
...
void db_buildportprotohashtable(NODEPROTO *cell)
{
	...
	INTBIG i, j, numportprotos, hashtablesize;
	//REGISTER PORTPROTO *pp;
	PORTPROTO *pp;
	...
	if (numportprotos != cell->numportprotos)
	{
		...
		ttyputmsg(_((char*)"Warning: number of ports in cell %s corrected"), describenodeproto(cell));
