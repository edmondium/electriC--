	...
	INTBIG a5, INTBIG a6)
{
	...
	static CHAR *broadcasttype[] = {M_((char*)"nodeinstnew"), M_((char*)"nodeinstkill"), M_((char*)"nodeinstmod"),
		M_((char*)"arcinstnew"), M_((char*)"arcinstkill"), M_((char*)"arcinstmod"), M_((char*)"portprotonew"),
		M_((char*)"portprotokill"), M_((char*)"portprotomod"), M_((char*)"nodeprotonew"), M_((char*)"nodeprotokill"),
		M_((char*)"nodeprotomod"), M_((char*)"objectstart"), M_((char*)"objectend"), M_((char*)"objectnew"), M_((char*)"objectkill"),
		M_((char*)"variablenew"), M_((char*)"variablekill"), M_((char*)"variablemod"), M_((char*)"variableinsert"),
		M_((char*)"variabledelete"), M_((char*)"descriptmod")};
	...
	if (db_broadcasting != 0)
	{
		...
		ttyputerr(_((char*)"Received this change during broadcast of type %s:"),
			TRANSLATE(broadcasttype[db_broadcasting-1]));			
		//ttyputmsg(x_("%s"), db_describechange(change, obj, a1, a2, a3, a4, a5, a6));
		ttyputmsg(x_((char*)"%s"), db_describechange(change, obj, a1, a2, a3, a4, a5, a6));
	}
...
CHANGEBATCH *db_preparenewbatch(void)
{
	...
	CHANGEBATCH *killbatch, *thisbatch;
	/* code cannot be called by multiple procesors: uses globals */
	NOT_REENTRANT;
...
BOOLEAN db_reversechange(CHANGE *c)
{
	...
	PORTPROTO *pp, *oldsubpp;
	//REGISTER NODEPROTO *np;
	NODEPROTO *np;
	//REGISTER NODEINST *ni, *oldsubni;
	NODEINST *ni, *oldsubni;
	//REGISTER ARCINST *ai;
	ARCINST *ai;
		...
		case VARIABLENEW:
			...
			if ((c->p3&VCREF) != 0)
			{
				...
				if (var == NOVARIABLE)
				{
					...
					ttyputmsg(_((char*)"Warning: Could not find attribute %s on object %s"),
						attname, describeobject(c->entryaddr, c->p1));						
					break;
				}
				c->p3 = var->addr;
			...
			if ((c->p4&VCREF) != 0)
			{
				...
				if (var == NOVARIABLE)
				{
					/*ttyputmsg(_("Warning: Could not set attribute %s on object %s"),
						attname, describeobject(c->entryaddr, c->p1));*/
					ttyputmsg(_((char*)"Warning: Could not set attribute %s on object %s"),
						attname, describeobject(c->entryaddr, c->p1));						
					break;
				}
			} else
		...
		case VARIABLEMOD:
			...
			if ((c->p3&VCREF) != 0)
			{
				...
				if (getind(c->entryaddr, c->p1, attname, c->p4, &oldval))
				{
					...
					ttyputmsg(_((char*)"Warning: Could not find index %ld of attribute %s on object %s"),
						c->p4, attname, describeobject(c->entryaddr, c->p1));						
					break;
				}
				nextchangequiet();
...
void showhistorylist(INTBIG which)
{
	...
	INTBIG node, arcinst, portproto, nproto, object, variable;
	//REGISTER INTBIG lobatch, hibatch;
	INTBIG lobatch, hibatch;
	//REGISTER CHANGEBATCH *batchreport;
	CHANGEBATCH *batchreport;
	//REGISTER CHANGE *c;
	CHANGE *c;
	CHAR line[50];
	...
	if (which >= 0)
	{
			...
			batchreport = batchreport->nextchangebatch)
		{
			...
			formatinfstr(infstr, M_((char*)"Batch %ld from %s tool"), batchreport->batchnumber,
				batchreport->tool->toolname);				
			if (batchreport->activity != 0)
				//formatinfstr(infstr, M_(", caused by '%s'"), batchreport->activity);
				formatinfstr(infstr, M_((char*)", caused by '%s'"), batchreport->activity);
			//addstringtoinfstr(infstr, M_(" is:"));
			addstringtoinfstr(infstr, M_((char*)" is:"));
			//ttyputmsg(x_("%s"), returninfstr(infstr));
			ttyputmsg(x_((char*)"%s"), returninfstr(infstr));
			for(c = batchreport->changehead; c != NOCHANGE; c = c->nextchange)
				/*ttyputmsg(x_("%s"), db_describechange(c->changetype, c->entryaddr, c->p1, c->p2,
					c->p3, c->p4, c->p5, c->p6));*/
				ttyputmsg(x_((char*)"%s"), db_describechange(c->changetype, c->entryaddr, c->p1, c->p2,
					c->p3, c->p4, c->p5, c->p6));					
			return;
		}
		//ttyputmsg(M_("Batch %ld is not in the list (range is %ld to %ld)"), which, lobatch, hibatch);
		ttyputmsg(M_((char*)"Batch %ld is not in the list (range is %ld to %ld)"), which, lobatch, hibatch);
		return;
	}

	/* display the change batches */
	//ttyputmsg(M_("%ld batches (limit is %ld):"), db_batchtally-1, db_maximumbatches);
	ttyputmsg(M_((char*)"%ld batches (limit is %ld):"), db_batchtally-1, db_maximumbatches);
	for(batchreport = db_tailbatch; batchreport != NOCHANGEBATCH;
		batchreport = batchreport->nextchangebatch)
	{
		...
		addstringtoinfstr(infstr, M_((char*)"} affects"));
		if (node != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), node, makeplural(M_("node"), node));
			(void)esnprintf(line, 50, x_(" %ld %s"), node, makeplural(M_((char*)"node"), node));
			addstringtoinfstr(infstr, line);
		}
		if (arcinst != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), arcinst, makeplural(M_("arc"), arcinst));
			(void)esnprintf(line, 50, x_(" %ld %s"), arcinst, makeplural(M_((char*)"arc"), arcinst));
			addstringtoinfstr(infstr, line);
		}
		if (portproto != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), portproto, makeplural(M_("port"), portproto));
			(void)esnprintf(line, 50, x_(" %ld %s"), portproto, makeplural(M_((char*)"port"), portproto));
			addstringtoinfstr(infstr, line);
		}
		if (nproto != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), nproto, makeplural(M_("cell"), nproto));
			(void)esnprintf(line, 50, x_(" %ld %s"), nproto, makeplural(M_((char*)"cell"), nproto));
			addstringtoinfstr(infstr, line);
		}
		if (object != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), object, makeplural(M_("object"), object));
			(void)esnprintf(line, 50, x_(" %ld %s"), object, makeplural(M_((char*)"object"), object));
			addstringtoinfstr(infstr, line);
		}
		if (variable != 0)
		{
			//(void)esnprintf(line, 50, x_(" %ld %s"), variable, makeplural(M_("variable"), variable));
			(void)esnprintf(line, 50, x_(" %ld %s"), variable, makeplural(M_((char*)"variable"), variable));
			addstringtoinfstr(infstr, line);
		}
		//ttyputmsg(x_("%s"), returninfstr(infstr));
		ttyputmsg(x_((char*)"%s"), returninfstr(infstr));
	...
	INTBIG p3, INTBIG p4, INTBIG p5, INTBIG p6)
{
	...
	switch (changetype)
	{
		case NODEINSTNEW:
			...
			formatinfstr(infstr, M_((char*)" Node '%s' created in cell %s"), describenodeinst(ni),
				describenodeproto(ni->parent));				
		...
		case NODEINSTKILL:
			...
			/*formatinfstr(infstr, M_(" Node '%s' killed in cell %s"), describenodeinst(ni),
				describenodeproto(ni->parent));*/
			formatinfstr(infstr, M_((char*)" Node '%s' killed in cell %s"), describenodeinst(ni),
				describenodeproto(ni->parent));				
			return(returninfstr(infstr));
		case NODEINSTMOD:
			ni = (NODEINST *)entryaddr;
			...
			formatinfstr(infstr, M_((char*)" Node '%s' changed in cell %s [was %sx%s, center (%s,%s), rotated %ld"),
				describenodeinst(ni), describenodeproto(ni->parent), latoa(p3-p1, lambda),
				latoa(p4-p2, lambda), latoa((p1+p3)/2, lambda), latoa((p2+p4)/2, lambda), p5);				
			//if (p6 != 0) addstringtoinfstr(infstr, M_(", transposed"));
			if (p6 != 0) addstringtoinfstr(infstr, M_((char*)", transposed"));
		...
		case ARCINSTNEW:
			...
			formatinfstr(infstr, M_((char*)" Arc '%s' created in cell %s"), describearcinst(ai),
				describenodeproto(ai->parent));				
			return(returninfstr(infstr));
		case ARCINSTKILL:
			ai = (ARCINST *)entryaddr;
			...
			/*formatinfstr(infstr, M_(" Arc '%s' killed in cell %s"), describearcinst(ai),
				describenodeproto(ai->parent));*/
			formatinfstr(infstr, M_((char*)" Arc '%s' killed in cell %s"), describearcinst(ai),
				describenodeproto(ai->parent));				
			return(returninfstr(infstr));
		case ARCINSTMOD:
			ai = (ARCINST *)entryaddr;
			...
			formatinfstr(infstr, M_((char*)" Arc '%s' modified in cell %s [was %s wide %s long from (%s,%s) to (%s,%s)]"),
				describearcinst(ai), describenodeproto(ai->parent), latoa(p5, lambda), latoa(p6, lambda),
					latoa(p1, lambda), latoa(p2, lambda), latoa(p3, lambda), latoa(p4, lambda));					
			return(returninfstr(infstr));
		case PORTPROTONEW:
			...
			formatinfstr(infstr, M_((char*)" Port '%s' created in cell %s"), pp->protoname,
				describenodeproto(pp->parent));				
			return(returninfstr(infstr));
		case PORTPROTOKILL:
			pp = (PORTPROTO *)entryaddr;
			...
			/*formatinfstr(infstr, M_(" Port '%s' killed in cell %s"), pp->protoname,
				describenodeproto(pp->parent));*/
			formatinfstr(infstr, M_((char*)" Port '%s' killed in cell %s"), pp->protoname,
				describenodeproto(pp->parent));				
			return(returninfstr(infstr));
		case PORTPROTOMOD:
			pp = (PORTPROTO *)entryaddr;
			...
			formatinfstr(infstr, M_((char*)" Port '%s' moved in cell %s [was on node %s, subport %s]"),
				pp->protoname, describenodeproto(pp->parent), describenodeinst((NODEINST *)p1),
				((PORTPROTO *)p2)->protoname);				
			return(returninfstr(infstr));
		case NODEPROTONEW:
			...
			formatinfstr(infstr, M_((char*)" Cell '%s' created"), describenodeproto(np));
			return(returninfstr(infstr));
		case NODEPROTOKILL:
			np = (NODEPROTO *)entryaddr;
			infstr = initinfstr();
			//formatinfstr(infstr, M_(" Cell %s' killed"), describenodeproto(np));
			formatinfstr(infstr, M_((char*)" Cell %s' killed"), describenodeproto(np));
			return(returninfstr(infstr));
		case NODEPROTOMOD:
			np = (NODEPROTO *)entryaddr;
			...
			formatinfstr(infstr, M_((char*)" Cell '%s' modified [was from %s<=X<=%s %s<=Y<=%s]"),
				describenodeproto(np), latoa(p1, lambda), latoa(p2, lambda),
					latoa(p3, lambda), latoa(p4, lambda));					
			return(returninfstr(infstr));
		case OBJECTSTART:
			...
			formatinfstr(infstr, M_((char*)" Start change to object %s"), describeobject(entryaddr, p1));
			return(returninfstr(infstr));
		case OBJECTEND:
			/* args: addr, type */
			infstr = initinfstr();
			//formatinfstr(infstr, M_(" End change to object %s"), describeobject(entryaddr, p1));
			formatinfstr(infstr, M_((char*)" End change to object %s"), describeobject(entryaddr, p1));
			return(returninfstr(infstr));
		case OBJECTNEW:
			/* args: addr, type */
			infstr = initinfstr();
			//formatinfstr(infstr, M_(" Object %s created"), describeobject(entryaddr, p1));
			formatinfstr(infstr, M_((char*)" Object %s created"), describeobject(entryaddr, p1));
			return(returninfstr(infstr));
		case OBJECTKILL:
			/* args: addr, type */
			infstr = initinfstr();
			//formatinfstr(infstr, M_(" Object %s deleted"), describeobject(entryaddr, p1));
			formatinfstr(infstr, M_((char*)" Object %s deleted"), describeobject(entryaddr, p1));
			return(returninfstr(infstr));
		case VARIABLENEW:
			...
			formatinfstr(infstr, M_((char*)" Variable '%s' created on %s"), changedvariablename(p1, p2, p3),
				describeobject(entryaddr, p1));				
			return(returninfstr(infstr));
		case VARIABLEKILL:
			...
			formatinfstr(infstr, M_((char*)" Variable '%s' killed on %s [was %s]"), changedvariablename(p1, p2, p4),
				describeobject(entryaddr, p1), describevariable(&myvar, -1, -1));				
			return(returninfstr(infstr));
		case VARIABLEMOD:
			/* args: addr, type, key, vartype, aindex, oldvalue */
			...
			/*formatinfstr(infstr, M_(" Variable '%s[%ld]' changed on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));*/
			formatinfstr(infstr, M_((char*)" Variable '%s[%ld]' changed on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));				
			if ((p3&VCREF) == 0)
			{
				...
				formatinfstr(infstr, M_((char*)" [was '%s']"), describevariable(&myvar, -1, -1));
			}
			return(returninfstr(infstr));
		case VARIABLEINS:
			/* args: addr, type, key, vartype, aindex */
			...
			/*formatinfstr(infstr, M_(" Variable '%s[%ld]' inserted on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));*/
			formatinfstr(infstr, M_((char*)" Variable '%s[%ld]' inserted on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));				
			return(returninfstr(infstr));
		case VARIABLEDEL:
			/* args: addr, type, key, vartype, aindex, oldvalue */
			...
			/*formatinfstr(infstr, M_(" Variable '%s[%ld]' deleted on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));*/
			formatinfstr(infstr, M_((char*)" Variable '%s[%ld]' deleted on %s"), changedvariablename(p1, p2, p3), p4,
				describeobject(entryaddr, p1));				
			return(returninfstr(infstr));
		case DESCRIPTMOD:
			infstr = initinfstr();
			/*formatinfstr(infstr, M_(" Text descriptor on variable %s changed [was 0%lo/0%lo]"),
				makename(p2), p4, p5);*/
			formatinfstr(infstr, M_((char*)" Text descriptor on variable %s changed [was 0%lo/0%lo]"),
				makename(p2), p4, p5);				
			return(returninfstr(infstr));
	}
	//return(x_("?"));
...
{
 /*  1 */ {0, {468,608,492,680}, BUTTON, N_((char*)"OK")},
 /*  2 */ {0, {32,8,455,690}, SCROLL, x_((char*)"")},
 /*  3 */ {0, {468,16,492,88}, BUTTON, N_((char*)"Undo")},
 /*  4 */ {0, {8,8,24,241}, MESSAGE, N_((char*)"These are the recent changes:")},
 /*  5 */ {0, {8,344,24,456}, CHECK, N_((char*)"Show details")},
 /*  6 */ {0, {468,108,492,180}, BUTTON, N_((char*)"Redo")}
};
//static DIALOG db_undodialog = {{50,75,555,775}, N_("Change History"), 0, 6, db_undodialogitems, 0, 0};
static DIALOG db_undodialog = {{50,75,555,775}, N_((char*)"Change History"), 0, 6, db_undodialogitems, 0, 0};
...
void db_loadhistorylist(INTBIG details, void *dia)
{
	...
	INTBIG node, arcinst, portproto, nproto, object, variable, lineno, curline;
	//REGISTER CHANGE *c;
	CHANGE *c;
	CHAR line[50];
	CHANGEBATCH *batch;
	...
	for(batch = db_tailbatch; batch != NOCHANGEBATCH; batch = batch->nextchangebatch)
	{
		...
		if (details != 0) addstringtoinfstr(infstr, M_((char*)"***** BATCH "));
