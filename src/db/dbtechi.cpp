...
BOOLEAN tech_doinitprocess(TECHNOLOGY *tech)
{
	...
	for(i=0; i<tech->nodeprotocount; i++)
	{
		...
		{
			ttyputerr(_((char*)"Cannot create nodeprotos"));
			return(TRUE);
		}
	...
	for(i=0; i<tech->arcprotocount; i++)
	{
		...
		{
			ttyputerr(_((char*)"Cannot create arcproto %s in technology %s"),
...
BOOLEAN tech_doaddportsandvars(TECHNOLOGY *tech)
{
	...
	for(i=0; i<tech->nodeprotocount; i++)
	{
		...
		for(j=0; j<nty->portcount; j++)
		{
			...
			{
				ttyputerr(_((char*)"Warning: node %s of technology %s shares port descriptions"),
			...
			if (portinst[j].addr == NOPORTPROTO)
			{
				ttyputerr(_((char*)"Error creating ports"));
				return(TRUE);
			}
	...
	for(i=0; tech->variables[i].name != 0; i++)
	{
		...
		if (namesame(tech->variables[i].name, x_((char*)"prototype_center")) == 0)
		{
...
void tech_convertmocmoslib(LIBRARY *lib)
{
	...
	ARCINST *ai;
	NODEINST *ni;
	TECHNOLOGY *mocmostech;

	for(mocmostech = el_technologies; mocmostech != NOTECHNOLOGY; mocmostech = mocmostech->nexttechnology)
		if (namesame(mocmostech->techname, x_((char*)"mocmos")) == 0) break;
	if (mocmostech == NOTECHNOLOGY)
	{
		ttyputmsg(M_((char*)"Cannot find 'mocmos' technology"));
		return;
	}
	...
	for(np = lib->firstnodeproto; np != NONODEPROTO; np = np->nextnodeproto)
	{
		for(ai = np->firstarcinst; ai != NOARCINST; ai = ai->nextarcinst)
		{
			...
			if (namesame(ai->proto->protoname, x_((char*)"S-Active")) != 0) continue;
			undogeom(ai->geom, np);
			ai->width += 4 * lambda;
			linkgeom(ai->geom, np);
			prims++;
		...
		for(ni = np->firstnodeinst; ni != NONODEINST; ni = ni->nextnodeinst)
		{
			...
			if (namesame(ni->proto->protoname, x_((char*)"S-Active-Pin")) == 0 ||
				namesame(ni->proto->protoname, x_((char*)"S-Transistor")) == 0 ||
				namesame(ni->proto->protoname, x_((char*)"Metal-1-S-Active-Con")) == 0)
			{
				undogeom(ni->geom, np);
				ni->lowx -= 2 * lambda;
				ni->highx += 2 * lambda;
	...
	if (prims == 0)
	{
		ttyputmsg(M_((char*)"No MOSIS CMOS objects found to alter"));
		return;
	}
	...
	ttyputmsg(M_((char*)"%ld S-active arcs, S-Active pins, Metal-1-S-Active"), prims);
	ttyputmsg(M_((char*)"  contacts, S-Transistors, and Metal-1-Substrate"));
	ttyputmsg(M_((char*)"  contacts from the MOSIS CMOS technology in this"));
	ttyputmsg(M_((char*)"  library have been adjusted."));
	ttyputmsg(M_((char*)"NOW DO A -debug check-database TO ADJUST CELL SIZES"));
}

void db_adjusttree(NODEPROTO *np)
{
	NODEINST *ni;
	INTBIG dlx, dhx, dly, dhy, nlx, nhx, nly, nhy;
...
{
	INTBIG lx, hx, ly, hy;
	INTBIG wid, swid, len, x1, y1, x2, y2, shrink1, shrink2, ang, end1, end2, dist,
		halflength;
	ARCINST *ai;
	PORTARCINST *pi, *pi1, *pi2;
...
void tech_fillpoly(POLYGON *poly, TECH_POLYGON *lay, NODEINST *ni, INTBIG lambda,
	INTBIG sty)
{
	INTBIG i, lastpoint, count;
	INTBIG x, y, *pt, xm, xs, ym, ys;
...
{
	INTBIG angle, ang;
	INTBIG thissg, next, total, segment, element, otherang;
	INTBIG sin, cos, xoff, yoff, thisxl, thisyl, thisxr, thisyr, lwid,
...
{
	INTBIG thissg, next, angle, pangle, ang;
	INTBIG sin, cos, thisx, thisy, nextx, nexty, pthisx, pthisy;
...
{
	INTBIG a;
	INTBIG aindex, sin, cos, innerradius, outerradius, wid;
	TECH_ARCLAY *thista;
...
{
	INTBIG temp, xextra, yextra, xe1, ye1, xe2, ye2, w2, sa, ca;
...
{
	ttyputmsg(_((char*)"Warning: arc %s cannot be negated: state reset"), describearcinst(ai));
...
VARIABLE *tech_filldisplayableanyvar(POLYGON *poly, INTBIG lx, INTBIG hx,
	INTBIG ly, INTBIG hy, INTBIG addr, INTBIG type, TECHNOLOGY *tech,
		WINDOWPART *win, NODEPROTO *cell, VARIABLE **varnoeval, POLYLOOP *pl)
{
	INTBIG len, language, whichindex;
