...
void osprimaryosinit(void)
{
	...
	TECHNOLOGY *tech, *lasttech, *realtech;
...
#  endif
	...
	for(i=0; el_constraints[i].conname != 0; i++)
	{
		...
		addstringtoinfstr(infstr, x_((char*)"constraint:"));
	...
	for(el_maxtools=0; el_tools[el_maxtools].toolname != 0; el_maxtools++)
	{
		...
		addstringtoinfstr(infstr, x_((char*)"tool:"));
	...
	for(el_maxtech=0; el_technologylist[el_maxtech].techname != 0; el_maxtech++)
	{
		...
		addstringtoinfstr(infstr, x_((char*)"tech:"));
		...
		if (realtech == NOTECHNOLOGY) error(_((char*)"No memory for technologies"));
		/* link it in */
	...
	for(tech = el_technologies; tech != NOTECHNOLOGY; tech = tech->nexttechnology)
	{
		if (tech->init != 0)
		{
			if ((*tech->init)(tech, 0))
				...
				error(_((char*)"User pass 1 error initializing %s technology"), tech->techname);
		}
		if (tech_doinitprocess(tech))
			//error(_("Pass 1 error initializing %s technology"), tech->techname);
			error(_((char*)"Pass 1 error initializing %s technology"), tech->techname);
	}
...
void ossecondaryinit(INTBIG argc, CHAR1 *argv[])
{
	...
	for(tech = el_technologies; tech != NOTECHNOLOGY; tech = tech->nexttechnology)
	{
		if (tech->init != 0)
		{
			if ((*tech->init)(tech, 1))
				...
				error(_((char*)"User pass 2 error initializing %s technology"), tech->techname);
		}
		if (tech_doaddportsandvars(tech))
			//error(_("Pass 2 error initializing %s technology"), tech->techname);
			error(_((char*)"Pass 2 error initializing %s technology"), tech->techname);
	}
	...
	for(tech = el_technologies; tech != NOTECHNOLOGY; tech = tech->nexttechnology)
	{
		if (tech->init != 0)
		{
			if ((*tech->init)(tech, 2))
				...
				error(_((char*)"User pass 3 error initializing %s technology"), tech->techname);
		}
	}
	...
	if (sizeof(INTHUGE) < 8)
		...
		error(x_((char*)"INTHUGE must be 64 bits wide (fix 'global.h')\n"));
	if (sizeof(INTBIG) < 4)
		//error(x_("INTBIG must be 32 bits wide (fix 'global.h')\n"));
		error(x_((char*)"INTBIG must be 32 bits wide (fix 'global.h')\n"));
	if (sizeof(INTSML) < 2)
		//error(x_("INTSML must be 16 bits wide (fix 'global.h')\n"));
		error(x_((char*)"INTSML must be 16 bits wide (fix 'global.h')\n"));
	if (sizeof(INTBIG) < sizeof(CHAR *))
		/*error(x_("INTBIG must be able to hold a pointer (%d bytes) bits wide (fix 'global.h')\n"),
			sizeof(CHAR *));*/
		error(x_((char*)"INTBIG must be able to hold a pointer (%d bytes) bits wide (fix 'global.h')\n"),
			sizeof(CHAR *));			
}
...
void toolturnon(TOOL *tool)
{
	...
	(void)setval((INTBIG)tool, VTOOL, x_((char*)"toolstate"), tool->toolstate|TOOLON, VINTEGER);
}
...
BOOLEAN stopping(INTBIG reason)
{
	...
	if (el_pleasestop == 1)
	{
		...
		ttyputmsg(_((char*)"%s stopped"), db_stoppingreason[reason]);
		el_pleasestop = 2;
	}
...
void ensurenonparallel(CHAR *file, INTBIG line)
{
	if (db_multiprocessing)
	{
		/*ttyputerr(_("ERROR: non-reentrant code (line %ld of module %s) called by multiple processors"),
			line, file);*/
		ttyputerr(_((char*)"ERROR: non-reentrant code (line %ld of module %s) called by multiple processors"),
			line, file);			
	}
}
		...
		*mutex = emakemutex();
		if (*mutex == 0)
		{
			...
			if (showerror)
				...
				ttyputmsg(_((char*)"Error creating mutual-exclusion lock"));
			return(TRUE);
		}
	}
	emutexunlock(*mutex);
