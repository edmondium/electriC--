...
BOXPOLY *db_mrg_do_break(BOXPOLY *me, BOXPOLY *him, INTBIG *rtc)
{
	...
	ttyputerr(_((char*)"Warning: merging done incorrectly"));
	ttyputerr(x_((char*)"me: left = %ld, right = %ld, top = %ld, bot = %ld"),
		me->left, me->right, me->top, me->bot);
	ttyputerr(x_((char*)"him: left = %ld, right = %ld, top = %ld, bot = %ld"),
...
void mergeaddmerge(void *vmerge, void *vaddmerge, XARRAY trans)
{
	...
	MERGE *merge, *addmerge;
	//REGISTER MPOLY *amp, *mp, *omp, *ump, *lastmp;
	MPOLY *amp, *mp, *omp, *ump, *lastmp;
	//REGISTER BOOLEAN done;
	BOOLEAN done;
...
CHAR *mergedescribe(void *vmerge)
{
	...
	for (mp = merge->mpolylist; mp != NOMPOLY; mp = mp->nextmpoly)
	{
		if (mp != merge->mpolylist) addstringtoinfstr(infstr, x_((char*)", "));
		formatinfstr(infstr, x_((char*)"%ld PTS on LAYER %ld"), mp->count, mp->layer);
	}
...
void mergeshow(void *vmerge, NODEPROTO *prim, INTBIG showXoff, INTBIG showYoff, CHAR *title)
{
	...
	INTBIG i, lx, hx, ly, hy, cx, cy, *newlist;
	merge = (MERGE *)vmerge;
	cell = getcurcell();
	if (cell == NONODEPROTO) return;
	for(mp = merge->mpolylist; mp != NOMPOLY; mp = mp->nextmpoly)
...
BOOLEAN db_mergepoly(MERGE *merge, MPOLY *mp, MPOLY *omp, BOOLEAN subtract)
{
	...
	INTBIG i, oi, j, startpol, startpt, intpoints;
...
BOOLEAN db_mergeaddholetopoly(MERGE *merge, MPOLY *holemplist, MPOLY *newmp)
{
	...
	INTBIG j, k, l, dist, bestdist, hint, nint, nx, ny, hfx, hfy, htx, hty, nfx, nfy, ntx, nty, lastj, lastk, solved;	
	INTBIG bestpos;
	...
	while (holemplist != NOMPOLY)
	{
		...
		if (solved == 0)
		{
			if (besthole == NOMPOLY)
			{
				...
				ttyputerr(x_((char*)"ERROR merging polygon: cannot remove hole"));
				break;
			}
			bestpos = nint+1;
...
int db_mergesortiny(const void *e1, const void *e2)
{
	...
	INTBIG i1, i2, lasti1, lasti2, y1, y2;
	i1 = *((INTBIG *)e1);
	i2 = *((INTBIG *)e2);
...
BOOLEAN db_mergeaddintersectionpoints(MERGE *merge, MPOLY *mp, MPOLY *omp)
{
	...
	INTBIG i, j, y, toty, oi, fy, ty, ofy, oty, last, olast, intvalue, cury, fromline, ofromline, startpos;	
	...
	MPOLY *omp, LISTINTBIG *Oqueue, LISTINTBIG *Oyorder, INTBIG ofromline, INTBIG intvalue)
{
	...
	INTBIG i, j, k, m, ic, fx, fy, tx, ty, ofx, ofy, otx, oty, last, olast, oi, icount, ix, iy, hasint, ohasint;	
	INTBIG ixa[2], iya[2];
...
BOOLEAN db_mergewalkedge(MERGE *merge, MPOLY *mp, INTBIG i, MPOLY *omp, MPOLY *newmp, BOOLEAN subtract)
{
	...
	INTBIG enterangle, leaveangle;
	/*REGISTER INTBIG startpol, startpt, last, next, pol, pt, unseenpt,
		unseenthis, smallestthisangle, smallestotherangle;*/
	INTBIG startpol, startpt, last, next, pol, pt, unseenpt, unseenthis, smallestthisangle, smallestotherangle;	
	INTBIG insert;
	...
	for(;;)
	{
		...
		if (newmp->count == 5000)
		{
			if (!merge->loopwarned)
			{
				...
				ttyputerr(_((char*)"Warning: loop detected while merging polygons on layer %s"),
					layername(mp->tech, mp->layer));					
...
void db_mergeaddintersectionpoint(MERGE *merge, INTBIG polygon, INTBIG point)
{
	...
	INTBIG i, newtotal, *newpol, *newpt, *newangle;
	...
	INTBIG ofx, INTBIG ofy, INTBIG otx, INTBIG oty, INTBIG *ix, INTBIG *iy)
{
	...
	INTBIG maxx, maxy, minx, miny, omaxx, omaxy, ominx, ominy, intpoints;
...
BOOLEAN db_mergeensurespace(MERGE *merge, MPOLY *mp, INTBIG count)
{
	...
	INTBIG i, *newx, *newy, *newseen, *newintersect, newlimit;
