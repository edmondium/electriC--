...
LIBRARY *newlibrary(CHAR *name, CHAR *file)
{
	...
	if (renamed) ttyputerr(_((char*)"Warning: library renamed to '%s'"), name);
...
void db_scalecell(NODEPROTO *np, INTBIG count, TECHNOLOGY **techarray, INTBIG *newlamarray)
{
	...
	INTBIG i, *oldlamarray, *position, oldlam, newlam;
	...
	if (i < count)
	{
		...
		var = getval((INTBIG)np, VNODEPROTO, VINTEGER|VISARRAY, x_((char*)"FACET_characteristic_spacing"));
		if (var != NOVARIABLE)
		{
			position = (INTBIG *)var->addr;
			position[0] = muldiv(position[0], newlam, oldlam);
...
void db_scalenodeinst(NODEINST *ni, INTBIG newlam, INTBIG oldlam)
{
	...
	INTBIG sizex, sizey, dx, dy, len, i, newlx, newhx, newly, newhy;
	...
	if (ni->proto->primindex == 0)
	{
		...
		if (dx != 0 || dy != 0)
		{
			...
			if (dx != 0 || dy != 0)
			{
				...
				ttyputmsg(_((char*)"Cell %s, node %s size and position adjusted"),
					describenodeproto(ni->parent), describenodeinst(ni));					
	...
	if (!inside0 || !inside1)
	{
		...
		} else
		{
			...
			} else
			{
				...
				ttyputmsg(_((char*)"Cell %s, arc %s no longer fixed-angle"),
					describenodeproto(ai->parent), describearcinst(ai));					
			}
		}
	}
...
void changeinternalunits(LIBRARY *whichlib, INTBIG oldunits, INTBIG newunits)
{
	...
	INTBIG hardscaleerror, softscaleerror, len;
	INTBIG num, den;
	//REGISTER INTBIG i;
	INTBIG i;
	//REGISTER CHAR *errortype, *errorlocation;
	CHAR *errortype, *errorlocation;
	//REGISTER TECHNOLOGY *tech;
	TECHNOLOGY *tech;
	//REGISTER LIBRARY *lib;
	LIBRARY *lib;
	...
	{
		//if (den > num) errortype = _("roundoff");
		if (den > num) errortype = _((char*)"roundoff");
		else
			//errortype = _("overflow");
			errortype = _((char*)"overflow");
		//if (hardscaleerror == 0) errorlocation = _("display");
		if (hardscaleerror == 0) errorlocation = _((char*)"display");
		else
		{
			//if (whichlib == NOLIBRARY) errorlocation = _("database");
			if (whichlib == NOLIBRARY) errorlocation = _((char*)"database");
			else
				//errorlocation = _("library");
				errorlocation = _((char*)"library");
		}
		//ttyputerr(_("Change caused %s errors in %s"), errortype, errorlocation);
		ttyputerr(_((char*)"Change caused %s errors in %s"), errortype, errorlocation);
		if (hardscaleerror != 0)
		{
			if (whichlib == NOLIBRARY)
			{
				//ttyputmsg(_("Recommend check of database for validity and keep old library files"));
				ttyputmsg(_((char*)"Recommend check of database for validity and keep old library files"));
				for(lib = el_curlib; lib != NOLIBRARY; lib = lib->nextlibrary)
					lib->userbits &= ~READFROMDISK;
			} else
			{
				//ttyputmsg(_("Recommend check of database for validity and keep old library file"));
				ttyputmsg(_((char*)"Recommend check of database for validity and keep old library file"));
				whichlib->userbits &= ~READFROMDISK;
			}
		} else
		{
			//ttyputmsg(_("Recommend redisplay"));
			ttyputmsg(_((char*)"Recommend redisplay"));
		}
	}
...
INTBIG db_scaleunits(INTBIG *value, INTBIG num, INTBIG den)
{
	...
	INTBIG orig, scaled, reconstructed;
	orig = *value;
