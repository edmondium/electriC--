...
void db_initclusters(void)
{
	...
	db_cluster = alloccluster(x_((char*)"DATABASE"));
#endif
	...
	el_tempcluster = alloccluster(x_((char*)"TEMPORARY"));
}
...
#endif
{
	...
	if (cluster == NOCLUSTER) ttyputerr(_((char*)"Warning: allocated %ld bytes from null cluster!"), amount);
...
void db_printclusterarena(CHAR *which)
{
	...
	ttyputmsg(_((char*)"Cluster option is not in use"));
#endif
}
