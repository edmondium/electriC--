...
BOOLEAN db_addtortnode(UINTBIG object, RTNODE *rtn, NODEPROTO *cell)
{
	...
	INTBIG i, oldcount, oldn, newn, bestoldnode, bestnewnode;
#ifdef	USEFLOATS
	...
	INTBIG bestoldexpand, bestnewexpand, newareaplus, oldareaplus, newarea, oldarea, scaledown;
#endif
	//REGISTER INTBIG olddist, newdist, dist;
	INTBIG olddist, newdist, dist;
	INTBIG lowestxv, highestxv, lowestyv, highestyv, lxv, hxv, lyv, hyv;
	//REGISTER RTNODE *newroot, *newrtn, *r;
	RTNODE *newroot, *newrtn, *r;
	...
	if (rtn->total >= MAXRTNODESIZE)
	{
		...
		if (oldcount != rtn->total + newrtn->total)
			...
			ttyputerr(_((char*)"R-trees: %ld nodes split to %d and %d!"),
				oldcount, rtn->total, newrtn->total);
...
void undogeom(GEOM *geom, NODEPROTO *parnt)
{
	...
	if (!db_findgeom(parnt->rtree, geom, &whichrtn, &whichind))
	{
		if (!db_findgeomanywhere(parnt->rtree, geom, &whichrtn, &whichind))
		{
			...
			ttyputerr(_((char*)"Internal warning: cannot find %s in R-Tree of %s"),
				geomname(geom), describenodeproto(parnt));				
			return;
		}
		/*ttyputmsg(_("Internal warning: %s not in proper R-Tree location"),
			geomname(geom));*/
		ttyputmsg(_((char*)"Internal warning: %s not in proper R-Tree location"),
			geomname(geom));			
	}
...
void db_removertnode(RTNODE *rtn, INTBIG ind, NODEPROTO *cell)
{
	...
	if (rtn->total < MINRTNODESIZE)
	{
		...
		if (i >= prtn->total) ttyputerr(_((char*)"R-trees: cannot find entry in parent"));
...
void db_printrtree(RTNODE *expectedparent, RTNODE *rtn, INTBIG indent)
{
	...
	if (rtn->parent != expectedparent) addstringtoinfstr(infstr, M_((char*)" WRONG-PARENT"));
	if (rtn->total > MAXRTNODESIZE) addstringtoinfstr(infstr, M_((char*)" TOO-MANY"));	
	if (rtn->total < MINRTNODESIZE && rtn->parent != NORTNODE)
		//addstringtoinfstr(infstr, M_(" TOO-FEW"));
		addstringtoinfstr(infstr, M_((char*)" TOO-FEW"));
	if (rtn->total != 0)
	{
		...
		if (rtn->lowx != lowestxv || rtn->highx != highestxv ||
			rtn->lowy != lowestyv || rtn->highy != highestyv)
				...
				addstringtoinfstr(infstr, M_((char*)" WRONG-BOUNDS"));
		rtn->lowx = lowestxv;
		rtn->highx = highestxv;
		rtn->lowy = lowestyv;
		rtn->highy = highestyv;
...
void boundobj(GEOM *geom, INTBIG *lx, INTBIG *hx, INTBIG *ly, INTBIG *hy)
{
	...
	INTBIG end0extend, end1extend, radius, angle, j, pts, xs, ys, xe, ye, xc, yc, xv, yv;	
	...
	for(;;)
	{
		...
		if (i < rtn->total)
		{
			...
			if (rs->depth >= MAXDEPTH-1)
			{
				...
				ttyputerr(_((char*)"R-trees: search too deep"));
				continue;
			}
			rs->depth++;
