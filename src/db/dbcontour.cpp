...
CONTOUR *gathercontours(NODEPROTO *np, NODEINST **usearray, INTBIG bestthresh, INTBIG worstthresh)
{
	...
	INTBIG sea, fdist, tdist, bestdist, dist, ex, ey, nodecount, foundthresh;
	INTBIG fx, fy, tx, ty;
	double startoffset, endangle;
	//REGISTER INTBIG l;
	INTBIG l;
	//REGISTER NODEINST *ni, *oni, *bestni;
	NODEINST *ni, *oni, *bestni;
	//REGISTER CONTOUR *con, *firstcontour;
	CONTOUR *con, *firstcontour;
...
BOOLEAN db_addelementtocontour(CONTOUR *con, NODEINST *ni)
{
	...
	INTBIG i, len, cx, cy, lastx, lasty, firstx, firsty;
...
void db_getcontoursegmentparameters(void)
{
	...
	var = getval((INTBIG)io_tool, VTOOL, VINTEGER, x_((char*)"IO_curve_resolution"));
	if (var == NOVARIABLE) db_arcres = DEFARCRES; else
		db_arcres = var->addr;
	//var = getval((INTBIG)io_tool, VTOOL, VINTEGER, x_("IO_curve_sag"));
	var = getval((INTBIG)io_tool, VTOOL, VINTEGER, x_((char*)"IO_curve_sag"));
...
void setcontoursegmentparameters(INTBIG arcres, INTBIG arcsag)
{
	db_arcres = arcres;
	db_arcsag = arcsag;
	(void)setval((INTBIG)io_tool, VTOOL, x_((char*)"IO_curve_resolution"), arcres, VINTEGER);
	...
	(void)setval((INTBIG)io_tool, VTOOL, x_((char*)"IO_curve_sag"), arcsag, VINTEGER);
}
...
void initcontoursegmentgeneration(CONTOURELEMENT *conel)
{
	...
	INTBIG swapx, swapy, radius, i, reverse;
	db_getcontoursegmentparameters();
